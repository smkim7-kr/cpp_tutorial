...
int main() {
	...
	char a = 65; char b = 'A'; // char can be initialized as number, but prints to character
	std::cout << "char variable can be initialized as number, but prints to character" << std::endl;
	std::cout << "char a = 65; char b = \'A\' prints into a = " << a << ", b = " << b << '\n' << std::endl; // 'A', 'A' 
	...
	std::cout << "bool variable prints to either 0 or 1" << std::endl;
	std::cout << "bool istrue = true; bool isfalse = false; prints into istrue = " << istrue << ", isfalse = " << isfalse << std::endl;
	
	std::cout << "Size of bool data type: " << sizeof(istrue) << " bytes." << std::endl; //sizeof() outputs byte size
	std::cin.get();
}